 ...
 * @since 2.0
 */
public abstract class TxpAbstractSqlParameterSource implements SqlParameterSource {
	...
	public void registerSqlType(String paramName, int sqlType) {
		...
		this.sqlTypes.put(paramName.toLowerCase(), sqlType);
	}
	...
	public void registerTypeName(String paramName, String typeName) {
		Assert.notNull(paramName, "Parameter name must not be null");
		this.typeNames.put(paramName.toLowerCase(), typeName);
	}
	...
	public int getSqlType(String paramName) {
		Assert.notNull(paramName, "Parameter name must not be null");
		Integer sqlType = this.sqlTypes.get(paramName.toLowerCase());
		if (sqlType != null) {
			return sqlType;
		}
		return TYPE_UNKNOWN;
	...
	public String getTypeName(String paramName) {
		Assert.notNull(paramName, "Parameter name must not be null");
		return this.typeNames.get(paramName.toLowerCase());
	}
