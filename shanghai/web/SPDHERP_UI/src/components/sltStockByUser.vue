...
export default {
    ...
    props: [ 'hosId', 'userId','inStocCode','isDefault','cmd'],
    methods: {
        getStocsByParam: function () {
            ...
            //根据父组件参数 判断是否需要默认选中
            if(this.isDefault){
            this.stockObj=[_temp.oneValue,_temp.twoValue];
            this.test(this.stockObj);
            }
        });
           ...
           stockInfo: {
            ...
            set: function (pathArr) {
                ...
                if (pathArr.length > 0) {
                    ...
                    var tmpArr = this.stockObj.concat();
