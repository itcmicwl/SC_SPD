<!--定向结算发票查询-->
<template>
        <div  style="height: 100%;" id="hosSettleDxDiv" @keyup.enter="getHosSettleForDX(1)">
        <el-col :span="24" class="toolbar" style="padding-bottom: 0px; padding-left:5px; height: 90%;">
            ...
            <template>
                <el-table ref="thisTable" :data="rowDatas"  row-key='id' :row-class-name="tableRowClassNameBill"
                    ...
                    <el-table-column prop="billId"  header-align="center"  label="结算单号" min-width="30">
                    ...
                    <el-table-column prop="status" align="center" label="状态"  width="60">
                        <template slot-scope="scope">
                            {{scope.row.status|SStatus}}
                        </template>
                    </el-table-column>
                    <el-table-column prop="hosName" header-align="center" align="center" label="请购机构" width="100"></el-table-column> 
                    ...
                    <el-table-column prop="invoiceCodes" align="center" label="发票编号" show-overflow-tooltip min-width="40"></el-table-column>
        ...
        <el-dialog title="开票详情" close="onClose" :visible.sync="queryFormVisible" :close-on-click-modal="true" width="75%">
            <el-table :data="settleDetails"  row-key='goodsId'
                ...
                <el-table-column prop="invoiceCode" align="center" header-align="center"  label="是否开票">
                    <template slot-scope="scope">
                            {{(scope.row.invoiceCode!=''&&scope.row.invoiceCode!='unundefined'&&scope.row.invoiceCode!=null)?'已开票':'未开票'}}
                    </template>
                </el-table-column>
                <el-table-column prop="invoiceCode" header-align="center" align="center" show-overflow-tooltip label="发票号"  width="80"></el-table-column>
                <el-table-column prop="invoiceAmount" align="center" header-align="center"  label="发票金额"></el-table-column>
                <el-table-column prop="invoiceMan" header-align="center" align="center" label="开票人" show-overflow-tooltip  width="80"></el-table-column>
                <el-table-column prop="invoiceDate" header-align="center" align="center" label="开票日期" show-overflow-tooltip width="80">
                    <template slot-scope="scope">
                            {{scope.row.invoiceCode?scope.row.invoiceDate:''}}
                    </template>
                </el-table-column>
                <el-table-column prop="invoiceType" header-align="center" align="center" label="发票类型" show-overflow-tooltip width="80"></el-table-column>
            </el-table>
        </el-dialog>
    </div>
</template>
<script>
    ...
    import { DISTRBILL_STATUS,SETTLE_STATUS } from '../../common/js/constance';
    export default {
        data() {
            return {
                hosId:'',
        ...
        components: { deptMonthGoodsReceiptQueryPrint },
        filters: {
        ...
        methods: {
            //获商品收货汇总列表
            getHosSettleForDX: function (pIndex) {
                this.page = pIndex;
                let eDate = null;
                ...
                this.loading = true;
                this.axios.post('/spdHERPService/reportAnalysis/reportStatistics/getHosSettleForDX', param).then(res => {
                    this.loading = false;
                    if (res.data.code == "0") {
            ...
            showDettail:function(rowData){
                ...
                this.getHosSettleDetailForDX(rowData);
            },
            getHosSettleDetailForDX: function (rowData) {
                var param = {
                    orderBy: "",
                    queryObject: {
                        pid:rowData.id
                ...
                this.axios.post('/spdHERPService/reportAnalysis/reportStatistics/getHosSettleDetailForDX', param).then(res => {
                    if (res.data.code == "0") {
            ...
            tableRowClassName (rowData, index) {
                 return (rowData.row.invoiceCode!=''
                    &&rowData.row.invoiceCode!='undefined'
                    &&rowData.row.invoiceCode!=null)? '' : 'red-row';
            },
            ...
            handleCurrentChange : function(val) {
                this.page = val;
                this.getHosSettleForDX(this.page);
            },
           ...
           initMessge:function(){
               this.spanMesg = moment(this.filters.sDate[0]).format("YYYY-MM-DD")+'到'+
                ...
                this.axios.post('/spdHERPService/reportAnalysis/reportStatistics/getHosSettleForDXMsg', param).then(res => {
                    if (res.data.code == "0") {
...
<style>
    #hosSettleDxDiv .el-checkbox__label{
        font-size: 12px;
        vertical-align: middle;
    }
    #hosSettleDxDiv .el-checkbox-group{
        vertical-align: middle;
    }
    #hosSettleDxDiv .deptgoods-tableheight {
        width: 100%;
        height:  calc(100% - 90px);
    }
    #hosSettleDxDiv .deptDetail-tableheight {
        width: 100%;
        height:  400px;
    }
    
    #hosSettleDxDiv .deptGoods{
        border-radius: 6px;
        padding-bottom: 10px;
        padding-top: 12px;
        padding-right: 19px;
    ...
    #hosSettleDxDiv .deptGoods .el-row{
        height: 36px;
    }
    
    #hosSettleDxDiv .pageBar{
        border-bottom: 1px solid #d1dbe5;
        border-left: 1px solid #d1dbe5;
        border-right: 1px solid #d1dbe5;
    }
    #hosSettleDxDiv .red-row {
         color: #ff2020;
    }
