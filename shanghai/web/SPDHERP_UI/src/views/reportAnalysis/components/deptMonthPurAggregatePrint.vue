   ...
   <el-row :gutter="10" id="billPrintContent" class="printArea"  v-show='false'>
      <el-row>
            <el-col :span="24" style="padding-bottom:10px" align="center" >
                ...
                <span><b>{{ sDate }}至{{eDate}}请购产品汇总</b></span>
            </el-col> 
        </el-row>
        <!--表格-->
        <el-row>
            <table class="gridtable">
                ...
                <tr v-for="(item,index) in goods" :key="item.id">
                    ...
                    <td align="center">{{item.qtyAggregate}}</td>
                </tr>
            </table>
        </el-row>
   </el-row>
...
export default {
  ...
  methods: {
    print(goods,dateRange,orgName){
        this.goods=goods;
        if(dateRange &&dateRange.length>1){
            this.sDate = moment(dateRange[0]).format("YYYY-MM-DD");
            this.eDate = moment(dateRange[1]).format("YYYY-MM-DD");
        }
