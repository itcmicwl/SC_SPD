  ...
  methods: {
    ...
    handleSlaverBarcodeEnter() {
      ...
      this.getBarcodeParseResult()
        .then(res => {
          this.parseBarcodeResultSet = new Set(
            res.map(item => {
              return {
        ...
        if (item.trackNo) {
          ...
          for (let row of rows) {
            ...
            if (row.qty === 0) {
              ...
              copiedRow.selected = JSON.parse(
                JSON.stringify(this.batchNoIdUniqueCodeList[copiedRow.goodsId].find(o => o.uniqueCode === item.trackNo))
              );
              copiedRow.qty = 1;
              rowAdded = true;
    ...
    getStockGoodsList() {
      return new Promise((resolve, reject) => {
        this.axios
          .get(this.baseURL + '/getStockGoodsList', {
            params: {
    ...
    newOutStockBill() {
      return {
        outStockKind: OUTSTOCK_BILL_KIND.MANUAL.value,
