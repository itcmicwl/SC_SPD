                              ...
                              <el-table-column prop="sourceWarehouseName" label="目标科室" sortable> </el-table-column>
                                ...
                                <el-table-column label="操作" width="150">
                                    <template slot-scope="scope">
                                        ...
                                        <el-button v-if="scope.row.state == 10||scope.row.state =='11'"  @click="auditBill(scope.row.billId)" type="text">审核</el-button>
                                  ...
                                  <el-table :data="buyBill.subVoList" style="width: 100%;" fit border height="300">
                                       <el-table-column label="操作" width="50">
                                        ...
                                        <el-table-column prop="qty" label="请购数量"  width="100"> 
                                            <template slot-scope="scope">
                                                    <el-input-number placeholder="请输入数量" v-model="scope.row.qty" size="mini" :min="0"  :step="scope.row.reqNum?scope.row.reqNum:1" class="col-input-num" :disabled="scope.row.isPacket==1" @change="qtyChange($event,scope.row)">
                                                    </el-input-number>
                                            </template>  
                                        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    auditBill(billId) {
      ...
      this.editFormVisible = true;
      this.getAuditBillDetails(billId);
    },
    delRow(pd) {
    ...
    submitBill: function(status) {
      ...
      this.axios
        .post(
          "/spdHERPService/applyMgr/purchaseAudit/submitBill",
          this.buyBill
        )
        .then(
          res => {
