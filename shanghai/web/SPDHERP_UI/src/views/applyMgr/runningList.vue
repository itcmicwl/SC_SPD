<template>
    <div id="running-list" style="height: 100%;">
            <el-col :span="24" style="height: 100%;">
                              ...
                              <el-table-column prop="sourceWarehouseName" label="目标科室" sortable> </el-table-column>
                                ...
                                <el-table-column label="操作" width="150">
                                    <template slot-scope="scope">
                                        ...
                                        <!--<el-button  v-if="scope.row.state == 10" @click="delBill(scope.row)" type="text">删除</el-button>-->
										<el-button v-if="scope.row.fillter==nowUserId&&scope.row.ifClosed==0"  @click="closeBill(scope.row.billId)" type="text">关闭</el-button>
                ...
                <el-dialog :title="modelTitle" close="onClose" :visible.sync="editFormVisible" :close-on-click-modal="false" width="70%" center>
                                  ...
                                  <el-table :data="buyBill.subVoList" style="width: 100%;" fit border height="300">
                                       <el-table-column label="操作" width="50">
                                        ...
                                        <el-table-column prop="applyQty" label="请购数量" width="100">
                                            <template slot-scope="scope">
                                                    <el-input-number placeholder="请输入数量" v-model="scope.row.applyQty" size="mini" :min="1" class="col-input-num" :disabled="scope.row.isPacket==1" @change="qtyChange($event,scope.row)">
                                                    </el-input-number>
                                            </template>
                                        </el-table-column>
                                        <el-table-column label="请购包数"  width="100">
                                            <template slot-scope="scope">
                                                <el-input-number placeholder="请输入数量"   v-model="scope.row.needPacketQty" size="mini" :min="0" class="col-input-num" :disabled="scope.row.isPacket==0||scope.row.isPacket==null" @change="qtyPacketChange($event,scope.row)">
                                                </el-input-number>
                                            </template>
                                        </el-table-column>
                                        ...
                                        <el-table-column prop="goodsGg" label="规格" width="200" show-overflow-tooltip>
                                        </el-table-column>
                                        ...
                                        <el-table-column prop="fieldCode2" label="件包" width="100" show-overflow-tooltip>
                                        </el-table-column>
                                        <el-table-column prop="provName" label="供应商" width="120" show-overflow-tooltip>
                                        </el-table-column>
                            ...
                            <span slot="footer" class="dialog-footer">
                                          <el-button :plain="true" type="warning"  @click="submitBill()">保存</el-button>
...
export default {
    ...
    methods: {
        	...
        	this.$confirm('是否确认关闭订单?', '提示', {
			   confirmButtonText: '确定',
		          ...
		          type: 'warning'
					  ...
					  this.axios.post('/spdHERPService/applyMgr/deptBuyMain/close/', params).then(res => {
						this.loading = true
						if (res.data.code == '0') {
        ...
        delRow(pd) {
        	if(this.buyBill.subVoList.length==1){
	    		this.$confirm('继续删除，保存时将作废该请购单, 是否继续?', '提示', {
		          confirmButtonText: '确定',
		          cancelButtonText: '取消',
		          type: 'warning'
		        }).then(() => {
        ...
        submitBill: function () {
         ...
         qtyChange(val, row) {
             row.applyQty = val;
             row.je = (row.price * row.applyQty).toFixed(2);
    ...
    mounted() {
		...
		this.nowUserId= this.user.userId;
        this.getDeptBuyList(1);
...
<style>
#running-list .el-table {
    font-size: 12px;
}
#running-list .RO-Main-tableheight{
