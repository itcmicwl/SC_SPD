<template>
  <el-dialog title="用户登陆" :visible.sync="showLoginDialog" size="tiny" @open="onOpen" :close-on-press-escape="false" :close-on-click-modal='false'>
        ...
        if (valid) {
          ...
          ulogin(loginParams).then(res => {
            ...
            if (res.flag === true) {
              ...
              _this.err.accountErr = res.accountErr;
              _this.err.passwordErr = res.passwordErr;
            }
          });
        }
      });
