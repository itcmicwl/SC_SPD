<template>
      <el-cascader :options="orgInfo"  v-model="Dept" :props="orgProps" :show-all-levels="false" :change-on-select="true">
      </el-cascader>
</template>
...
export default {
    ...
    methods: {
        ...
        getOrgInfo(hosId) {
          ...
          var _this =this;
            var params = { corpId:hosId };
            this.axios.post('/platformService/sys/org/getOrgsByUnit', params).then(res=>{
                if (res.data.code == "0") {
                    if(res.data.data && Array.isArray(res.data.data.children)){
