<template>
    <div  style="height: 100%;">
            <el-col :span="24" style="height: 100%;">
                ...
                <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
                    <el-button-group>
                        <el-button type="primary" icon="edit" size="mini" @click.native="addReturn">导入退货单</el-button>
                    </el-button-group>
                </el-col>
                <!--列表-->
                <template>
                ...
                <template>
                     <el-dialog :title="modelTitle" close="onClose" :visible.sync="editFormVisible" :close-on-click-modal="false" size='large'>
                            ...
                            <template>
                                <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
                                    <el-button-group>
                                        <el-button type="primary" class="el-icon-plus" size="mini" @click.native="getReturnBillData">从收货单中选择</el-button>
                                    </el-button-group>
                                </el-col>
                                <el-table :data="returnBill.returnBillSubList" style="width: 100%; height:100%;" fit  border>
                                   ...
                                   <el-table-column prop="goodsStatus" label="质量状态" >
                                    ...
                                    <el-table-column prop="canReturnQty" label="可退数量" width="80px" >
                                        ...
                                        <el-table-column label="退货数量" >
                                            <template slot-scope="scope">
                                                <el-input-number placeholder="请输入数量" v-model="scope.row.qty" size="mini" :min="0" :max="scope.row.canReturnQty"  class="col-input-num"  @change="qtyChange($event,scope.row)">
                                                </el-input-number>
                                            </template>
                                       </el-table-column>
                   ...
                   <!-- 收货单列表开始-->
                <template>
                        <el-dialog title="从收货单中选择" close="onClose" :visible.sync="receiveFormVisible" :close-on-click-modal="false" size='small'>

                            <el-table :data="receiveBillList" style="width: 100%; height: 30%;"
                                        highlight-current-row
                                        @current-change="handleCurrentChange4Row"
                                        fit  border
                                     >
                                    <!--
                                <el-table-column prop="rowNumber" label="序号" width="60">
                                </el-table-column>-->
                                <el-table-column prop="billId" label="单号" >
                                </el-table-column>
                                <!--
                                <el-table-column prop="purchaseCompanyName" label="请购医院" width="150">
                                </el-table-column>
                                <el-table-column prop="applyOrgName" label="请购科室" width="150">
                                </el-table-column>
                                <el-table-column prop="recieveOrgName" label="收货科室" width="150">
                                </el-table-column>
                                -->
                                <el-table-column prop="provName" label="配送单位"  show-overflow-tooltip>
                                </el-table-column>
                                <el-table-column prop="revieverName" label="验收人"  show-overflow-tooltip>
                                </el-table-column>
                                <!--
                                <el-table-column prop="fillterName" label="制单人" width="180" show-overflow-tooltip>
                                </el-table-column>
                                <el-table-column prop="memo" label="说明" width="180" show-overflow-tooltip>
                                </el-table-column>
                                -->
                                ...
                                <el-table-column label="笔数" width="90">
                                    <template slot-scope="scope">
                                        <el-input-number placeholder="请输入数量"  :disabled=true  v-model="scope.row.sumRow" size="mini" :min="0" class="col-input-num" @change="qtyChange($event,scope.row)">
                                        </el-input-number>
                                    </template>
                                 </el-table-column>
                            </el-table>
                            <el-col :span="24" class="toolbar">
                                <el-pagination @size-change="handleReceiveSizeChange" @current-change="handleReceiveCurrentChange" :current-page="receivePage" :page-sizes="[10, 20, 30, 50]" :page-size="receivePageSize" layout="total, sizes, prev, pager, next" :total="receiveTotal" style="float:right;" small>
                                </el-pagination>
                            </el-col>

                           	<el-table :data="receiveBillSubList" highlight-current-row v-loading="loading"
                               style="width: 100%;  height: 70%;" fit  border
                               row-key='id'  @selection-change="handleSelectionChange4receiveBillSub">
                                    <!--	<el-table-column type="index" label="序号" align="center" width="62"></el-table-column>     -->
                                        <el-table-column  type="selection" :reserve-selection="true" width="55"> </el-table-column>
                                          <!--
                                        <el-table-column prop="purBillId" label="采购订单号" header-align="center" width="90"></el-table-column>

                                        <el-table-column prop="purBillDate" label="采购订单日期" header-align="center" min-width="180" sortable></el-table-column>

                                        <el-table-column prop="applyBillId" label="科室请购单号" header-align="center" width="100"></el-table-column>
                                        <el-table-column prop="saleBillId" label="销售单号" header-align="center" width="150"></el-table-column>
                                         -->
                                        ...
                                        <el-table-column prop="qty" min-width='25%' label="数量"   header-align="center" ></el-table-column>
                                        <!--
                                        <el-table-column prop="unrecieveQty" label="待验收数量" header-align="center" width="65"> </el-table-column>
                                        -->
                            </el-table>
                            <!--工具条-->
                            <el-col :span="24" class="toolbar">
                                <el-pagination @size-change="handleSizeChange4receiveBillSub" @current-change="handleCurrentChange4receiveBillSub" :current-page="receiveSubPage" :page-sizes="[10, 20, 30, 50]" :page-size="receiveSubPageSize" layout="total, sizes, prev, pager, next" :total="receiveSubTotal" style="float:right;" small>
                                </el-pagination>
                            </el-col>
                            <div slot="footer" class="dialog-footer">
                                <el-button @click.native="receiveFormVisible = false">取消</el-button>
...
export default {
    data() {
        return {
            ...
            ProvsInfo: [],
            provs4Balance:[],
            returnBill:{id:'',billId:'',fillDate:'',returnCompanyId:'',returnCompanyName:'',
                        returnOrgId:'',returnOrgCode:'',returnOrgName:'',
                        outOrgId:'',   outOrgName:'',   outOrgCode:'',
                        provId:'',provName:'',
                        returnBillSubList: []},
            returnBillList: [],
            returnBillSub:{id:'',pid:'',rowNumber:'',returnBillId:'',hosGoodsId:'',batchNum:'',
                           batchCode:'',isUniqueCode:'',hosGoodsCode:'',hosGoodsName:'',
                           goodsStatus:'',instorePrice:'',outstorePrice:'',canReturnQty:'',
                           qty:'',purBillId:'',applyBillId:'',returnReason:'',version:'',lstUniqueList:[]},
            ...
            multipleSelection4receiveBillSub:[],
    ...
    methods: {
       ...
       returnBillView(billId) {
        ...
        // this.returnBill={}; 太粗暴了
         ...
         //   this.opModule ="ADD";
            ...
            let serviceUrl = this.returnBill.billId ? '/spdHDIService/orderMgr/returnBill/updateReturnBill' : '/spdHDIService/orderMgr/returnBill/addReturnBill';
         ...
         delReturn(returnBill) {
            this.$confirm('确认删除这条记录吗?', '提示', {
                ...
                this.loading = true;
                this.axios.post("/spdHDIService/orderMgr/returnBill/delReturnBill", returnBill).then(res => {
                    this.loading = false;
                    if (res.data.code == "0") {
                        this.loading = false;
       ...
       getReturnBill(billId) {
        ...
        delRow(pd) {
            var index = this.returnBill.returnBillSubList.findIndex(item => { return item.rowNumber == pd.rowNumber });
            this.returnBill.returnBillSubList.splice(index, 1);
            this.returnBill.returnBillSubList.forEach((item, i) => item.rowNumber = i + 1);
        },
        ...
        getReceiveBillList:function(pIndex){
            this.receivePage = pIndex;
             var param = {
                orderBy: "",
                page: this.receivePage,
                rows: this.receivePageSize,
                queryObject: {
            ...
            this.axios.post('/spdHDIService/orderMgr/receiveBill/listByPage', param).then(res => {
                ...
                this.receiveTotal = res.data.data.total;
            }, err => {
                this.loading = false;
            });
         ...
         handleCurrentChange4Row(val) {
            ...
            if(val!=null) {
                this.currentRow = val;
            //       this.returnBill.billId = val.billId;

                this.getReceiveBillSubList(1)
            }


           },

           getReceiveBillSubList:function(pIndex){

             this.receiveSubPage = pIndex;
            var param = {
                orderBy: "",
                page: this.receiveSubPage,
                rows: this.receiveSubPageSize,
                queryObject: {
                  //  billId: this.currentRow.billId,
                    pid:this.currentRow.id
                }
            };

              //加载收货子表数据
            this.axios.post(`/spdHDIService/orderMgr/receiveBill/listSubReceiveByPage`, param).then(res => {
                if (res.data.code == "0") {
                     this.receiveBillSubList=res.data.data.data;
                     this.receiveSubTotal = res.data.data.total;
        ...
        selectSubmit:function(){
              ...
              if (this.multipleSelection4receiveBillSub.length > 0) {
                  this.multipleSelection4receiveBillSub.forEach(item => {
       ...
       cancel() {
        ...
        getProvs4Balance(){
            let param = { orderBy: "", queryObject: {hosId:this.user.corpId ,provId:this.returnBill.provId}, page: 0, rows: 0 };
            this.axios.post('/spdHDIService/collector4orderMgr/hosCollectorProv/listVoByPage', param).then(res => {
                this.loading = false;
        ...
        handleSizeChange(val) {
         ...
         handleReceiveCurrentChange(val) {
            this.receivePage = val;
            this.getReceiveBillList(this.receivePage);
        },
        handleReceiveSizeChange(val) {
            this.receivePage = val;
            this.getReceiveBillList(this.receivePage);
        },
        handleCurrentChange4receiveBillSub(val) {
            this.receiveSubPage = val;
            this.getReceiveBillSubList(this.receiveSubPage);
        },
        handleSizeChange4receiveBillSub(val) {
            this.receiveSubPageSize = val;
            this.getReceiveBillSubList(this.receiveSubPageSize);
        },
         handleSelectionChange4receiveBillSub(val) {
                this.multipleSelection4receiveBillSub = val;
         },
        ...
        // }
          getZLZT: function () {   //质量状态 字典
                this.loading = true;
                  this.axios.post('/platformService/sys/dict/getDictValueByDictEname',{dictName:'KQZLZT'}).then(res => {                    this.loading = false;
                    if (res.data.code == "0") {
    ...
    this.getZLZT();
        ...
        if (billId) {
          ...
          //  Methods.push(_this.getPurchaseInfo(billId));
            ...
            _this.returnBill.fillDate = util.formatDate.format(new Date(), "yyyy-MM-dd");
        }
        _this.axios.all(Methods).then(_this.axios.spread(function (orgInfo, provInfo) {
