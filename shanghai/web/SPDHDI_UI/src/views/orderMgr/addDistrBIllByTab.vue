    ...
    <el-row :gutter="10" v-loading="pageLoading" style="height:100%;">
        <el-col :span="24" style="height:100%;">
            ...
            <el-table :data="distrBill.distrBillList" style="width: 100%;" fit border class="table_sub_content">
                        ...
                        <el-form label-width="120px" class="el-form-item-nomsg">
                            ...
                            <el-col :span="12">
                                <el-form-item label="有效日期">
                                    <el-date-picker v-model="props.row.expdtEndDate" type="date" :picker-options="pickOpExpdt" placeholder="选择日期" >
                                    </el-date-picker>
                                </el-form-item>
                            </el-col>
                            <el-col :span="12">
                            ...
                            <el-col :span="12">
                                <el-form-item label="出库库区">
                                    <el-input v-model="props.row.outStorearea" placeholder="出库库区" ></el-input>
                                </el-form-item>
                            </el-col>
                            <!-- <el-col :span="12">
                                <el-form-item label="出库类型">
                                    <el-input v-model="props.row.outKind" placeholder="出库类型" ></el-input>
                                </el-form-item>
                            </el-col>
                            <el-col :span="12">
                                <el-form-item label="换货标识">
                                    <el-input v-model="props.row.isExchange" placeholder="换货标识" ></el-input>
                                </el-form-item>
                            </el-col> -->
                            <el-col :span="12">
                                <el-form-item label="定量包装">
                                    <el-input-number v-model="props.row.packetCode"  :min="0"></el-input-number>
                                </el-form-item>
                            </el-col>
                            <el-col :span="12">
                            ...
                            <el-col :span="12">
                                <el-form-item label="送货日期">
                                    <el-date-picker v-model="props.row.deliveryDate" placeholder="灭菌批号" ></el-date-picker>
                                </el-form-item>
                            </el-col>
                            <el-col :span="12">
                ...
                <el-table-column label="发货单价">
                    <template slot-scope="scope">
                        <el-input v-if="distrBill.provId == distrBill.subProvId" v-model="scope.row.sendPrice"  @blur="checkPrice($event,scope.row)">
                        </el-input>
                        <el-input v-if="distrBill.provId != distrBill.subProvId" v-model="scope.row.subSendPrice"  @blur="checkPrice($event,scope.row)">
                        </el-input>
                    </template>
                </el-table-column>
               ...
               <el-table-column label="注册证号">
                ...
                <el-table-column label="有效日期">
                    <template slot-scope="scope">
                        <el-date-picker v-model="scope.row.expdtEndDate" type="date" :picker-options="pickOpExpdt" placeholder="选择日期" >
                        </el-date-picker>
                    </template>
                </el-table-column>
            ...
            <el-dialog title="其他信息设置" :visible.sync="otherV" :close-on-click-modal="false"  @close="distrDetailClose">
                <el-form label-width="120px" class="el-form-item-nomsg">
                    ...
                    <el-col :span="12">
                        <el-form-item label="有效日期">
                            <el-date-picker v-model="distrDetail.expdtEndDate" type="date" :picker-options="pickOpExpdt" placeholder="选择日期" >
                            </el-date-picker>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
...
export default {
        ...
        return {
            ...
            pickOpExpdt:{
                disabledDate:(curDate)=>{
                    let flag = moment().isAfter(curDate);
                    return flag;
                }
            },
            barcodeMatch: {
    ...
    methods: {
        ...
        convertPurcToDistr(purc) {
            ...
            this.distrBill.subProvCode = purc.subProvCode;
            this.distrBill.subProvName = purc.subProvName;
            this.distrBill.subProvErpCode = purc.subProvErpCode;

            this.distrBill.remark = '';
            this.distrBill.sumRow = purc.lstDetail.length;
            this.distrBill.status = 10;
            this.distrBill.isConverted = (this.distrBill.subProvId==this.distrBill.provId) ? 1:0;
           ...
           // this.distrBill.provId = this.user.corpId;
            ...
            if (purc.lstDetail.length > 0) {
                purc.lstDetail.forEach(item => {
                    ...
                    //row.sendPrice = item.provGoodsErpPrice | 0;
        ...
        checkSubmit(distrBill){
            ...
            var errMsg=[];
            distrBill.distrBillList.forEach((item,index)=>{
                if(Number(item.sendQty)<=0){
                    errMsg.push(`第${index}行，${item.provGoodsErpName}的<font color='red'>配送数量</font>必须大于零`);
                }
                if(!item.batchCode){
                    errMsg.push(`第${index}行，${item.provGoodsErpName}的<font color='red'>批号</font>不能为空`);
                }
                if(!item.expdtEndDate){
                    errMsg.push(`第${index}行，${item.provGoodsErpName}的<font color='red'>有效期</font>不能为空`);
                }
                if(Number(item.sendPrice)<=0){
                    errMsg.push(`第${index}行，${item.provGoodsErpName}的<font color='red'>配送价</font>必须大于零`);
                }

                if(Number.isNaN(Number(item.sendTemprature)) || Number.isNaN(Number(item.sendHumidity))){
                    errMsg.push(`第${index}行，${item.provGoodsErpName}的<font color='red'>温湿度</font>必须为数字`);
                }
            });
            if(errMsg.length>0){
...
/* 			params.distrBillList.forEach(item=>{
			    item.subSendPrice = item.sendPrice
			}); */
            this.pageLoading = true;
