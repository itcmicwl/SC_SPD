<template>
    <div id="barcodeParse" class="fullheight">
        <el-row style="margin-bottom: 5px;">
            <el-input ref="searchInput" placeholder="请输入条码" v-model="searchBarcode" 
                      @keyup.enter.native="searchBtnClick" style="width:30%;">
                <el-button slot="append" icon="el-icon-search" @click="searchBtnClick">查询</el-button>
            </el-input>
        </el-row>
        <el-row class="tableheight">
            <el-table highlight-current-row class="fullheight" ref="parseResultTable"
                      :data="parseResult" style="width: 100%" v-loading="loading" border>
                <el-table-column label="条码" show-overflow-tooltip prop="barcode" fixed="left"
                ...
                <el-table-column label="条码类型" :formatter="formatType" show-overflow-tooltip
                                 prop="mainFlag"></el-table-column>
...
<script>
    export default {
        data () {
            return {
                baseURL: '/spdHDIService/baseData/basBarcode',
                searchBarcode: '',
                parseResult: [],
                loading: false,
            }
...
<style>
    ...
    #barcodeParse .expiring {
        color: #ff00ff;
    }

    #barcodeParse .expired {
        color: red;
    }
