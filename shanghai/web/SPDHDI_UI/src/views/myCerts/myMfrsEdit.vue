  ...
  export default {
    ...
    methods: {
      ...
      updateCerts () {
        ...
        let serverUrl = "/spdHDIService/myCerts/myMfrs/updateMfrsCerts";
      ...
      addImage (imageObj) {
        let serverUrl = "/spdHDIService/myCerts/myMfrs/addImage";
      ...
      deleteProvCerts:function (row) {
        this.$confirm('确认删除吗?', '提示', {}).then(() => {
          this.axios.post('/spdHDIService/myCerts/myMfrs/deleteMfrsCerts', row).then(res => {
            if (res.data.code == "0") {
      ...
      deleteCert:function (file, fileList) {
        ...
        this.axios.post('/spdHDIService/myCerts/myMfrs/deleteImage', {id:file.id}).then(res => {
          if (res.data.code == "0") {
            this.$message({message: '操作成功！', type: 'success'});
      ...
      dlgConfirm () {
        this.$refs.cnsForm.validate((valid) => {
          if (valid) {
            this.dlgConfirmBtnLoading = true;
            let serverUrl = "/spdHDIService/myCerts/myMfrs/addMyMfrsCerts";
            this.companyCerts.pId = this.company.id;
