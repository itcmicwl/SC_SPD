<template>    
    <div id="corps-roles-config" style="height: 100%;padding:0 3px ">
        ...
        <el-col :span="10" style="height: 100%;">
           <el-card class="mycard">
                ...
                <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
                    <el-select v-model="companies.selectedCompanyId" ref="selectedCompany" @change="loadCompanyRoles" filterable placeholder="请选择公司名称：" style="width:100%">
                        <el-option class="my-select" v-for="item in companies.data" :key="item.id" :label="item.cname" :value="item.id">
                            <div style="font-size: 12px">{{ item.cname }}</div>
                        </el-option>
                    </el-select>
                </el-col>
                <el-table highlight-current-row   v-loading="loading" border tooltip-effect="dark"  style="height: calc(100% - 60px);width: 100%;"
                    ...
                    <el-table-column label="类型" width="80" header-align="center" >
                        <template slot-scope="scope"> 
                            <span style="color:red" v-if="scope.row.kind == '11'" >管理员</span>
                            <span style="color:green" v-else-if="scope.row.kind == '12'" >用户</span>
                        </template>
                    </el-table-column>
        ...
        <el-dialog :title="dialog.title" :visible.sync="dialog.visible" :modal="dialog.modal" :show-close="false"
            ...
            <el-form ref="roleForm" :model="dialog.roleForm" label-width="80px">
                ...
                <el-form-item label="类型" class="my-form-item">
                    <span style="color:red" v-if="dialog.roleForm.kind == '11'" >管理员角色</span>
                    <span style="color:green" v-else-if="dialog.roleForm.kind == '12'" >用户角色</span>
                </el-form-item>
...
var user={};
    export default {
        ...
        methods: {            
            onAddNewRole(){
                let corpId = this.companies.selectedCompanyId;
                if(!corpId){
                    o.$message.error("还没有选择一个公司或机构！");
                    return ;
                }
                let row = {id:'0',code:'',companyId:corpId,kind:'12',cname:'新增角色1',remark:'新增角色',roleTemplateId:'',flag:'1',version:0};
            ...
            loadCompanies(currentCompanyId){
                let o = this;
                o.loading = true;
                axios.post(baseUrl + `/platformService/permission/adminConfig/listCompanies`).then(res => {
                    o.loading = false;
                    if (res.data.code == 0) {   //成功
                       o.companies.data =  res.data.data;
                       if(res.data.data.length>0){
                           o.companies.selectedCompanyId = res.data.data[0].id
                           o.loadCompanyRoles(res.data.data[0].id);
                       }
                    } else{
                        o.$message.error(res.data.msg);
                    }
...
<style>
    ...
    *{
    ...
    #corps-roles-config .el-tree--highlight-current .el-tree-node.is-current>.el-tree-node__content {
        background-color: #8cb8d8;
    }
    #corps-roles-config .el-select-dropdown__item.selected{
        
    }
    #corps-roles-config .el-table .cell, .el-table th>div {
        padding-left: 10px;
        padding-right: 10px;
    }
    #corps-roles-config .admin-flag{
        text-align:center;
        width: 100%;
        display: inline-block; 
        color:red;
    }   
    #corps-roles-config .my-select{
        height:32px;
        
    }
    #corps-roles-config .el-tree-node__label {
        font-size: 12px;
    }
    #corps-roles-config .el-tree {
        border-radius: 4px;
    }
    #corps-roles-config .el-tabs__item 
    {
        font-size: 12px;
    }
    /*.el-dialog__header {
    ...
    #corps-roles-config .my-dialog-body
    {
        padding: 15px;
         width:1000px;
    ...
    #corps-roles-config .my-dialog-body .label{
        padding:0 0 4px;
    ...
    #corps-roles-config .my-label
    {
        padding:10px;        
    }
    #corps-roles-config .my-label span{
        font-size:14px;
    }
    #corps-roles-config .tree-outerline
    {
        border: 0px solid lightgray;
        border-radius: 4px;
        padding:5px 0;
    }  
    #corps-roles-config .my-form-item
    {
        margin-bottom: 16px;
    }
    #corps-roles-config .dialog-title
    {
        display:inline-block;
        font-size:16px;
        margin: 0 0 10px;
    }
    #corps-roles-config .my-node-icon{
        color: #1d6ce6;
    }
    #corps-roles-config .my-node-summary{
        display: inline-block;
        margin-left:5px; 
        color: green;
    }
    #corps-roles-config .my-check{
        display: inline-block;
        width: 15px;
        height: 11px;
   ...
   #corps-roles-config .my-treenode-label{
       display: inline-block;
   }
</style>