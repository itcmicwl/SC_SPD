  ...
  <div id="base-dict-maint" class="full-height">
    <el-row class="full-height">
      <el-col :span="6" class="full-height">
        <el-input placeholder="搜索" icon="search" v-model="searchParam" @click="getDictList(1)" class="search-input">
        </el-input>
        ...
        <el-table :data="dictListData" highlight-current-row @current-change="handleCurrentChange" style="width: 100%;height: calc(100% - 80px)" height="css" border >
        ...
        <el-col :span="24" class="toolbar">
          <el-pagination @size-change="handleSysDictSizeChange" @current-change="handleSysDictCurrentChange"
                         :current-page="page" :page-sizes="[20, 50, 100, 200]"
      ...
      <el-col :span="18" style="padding-left: 10px" class="full-height">
        ...
        <el-table :data="dictValueListData" highlight-current-row @current-change="handleCurrentChange4Right" style="width: 100%;" border>
          <el-table-column type="index" width="50">
          </el-table-column>
          <el-table-column property="dictId" width="150" label="字典ID">
          </el-table-column>
          <el-table-column property="dictName" width="150" label="字典名">
          </el-table-column>
          <el-table-column property="dictCname" width="150" label="字典别名">
          </el-table-column>
          <el-table-column property="val" width="150" label="值编码">
          </el-table-column>
    ...
    <el-dialog :title="popTitle" :visible.sync="editFormVisible">
      <el-form :model="dict" :inline="true" label-width="80px" :rules="editFormRules" ref="editDictForm">
        ...
        <el-form-item label="类别" prop="kind">
          ...
          <el-select v-model="dict.kind" placeholder="请选择" style="width:88%">
...
export default {
  data() {
    return {
      ...
      editFormRules: {
        ename: [{ required: true, message: "请输入字典key", trigger: "blur" }]
      },
      editForm4RightRules: {
        ename: [
  ...
  methods: {
    ...
    getDictList: function(pIndex) {
      ...
      this.loading = true;
      this.axios.post("/platformService/sys/dict/getDictList", param).then(
        res => {
          this.loading = false;
          this.dictListData = res.data.data.data;
    ...
    handleSysDictSizeChange(val) {
      this.pageSize = val;
      this.getDictList(this.page);
    },
    handleSysDictCurrentChange(val) {
      this.page = val;
      this.getDictList(this.page);
    },
    ...
    handleSearchIconClick() {
      if (this.searchParam) {
        this.dictListData = JSON.parse(
          JSON.stringify(
            this.dictListOrigData.filter(item => {
              return (
                item.ename.indexOf(this.searchParam) !== -1 ||
                item.cname.indexOf(this.searchParam) !== -1
              );
            })
          )
        );
    ...
    executeDel: function() {
      this.axios
        .post(`/platformService/sys/dict/delete`, { id: this.currentRow.id })
        .then(
          res => {
    ...
    editDictSubmit: function() {
      this.$refs.editDictForm.validate(valid => {
        if (valid) {
          ...
          this.axios.post(`/platformService/sys/dict/update`, this.dict).then(
            res => {
              this.editLoading = false;
              if (res.data.code === 0) {
                this.$message({ message: "操作成功！", type: "success" });
    ...
    showDictList: function() {
      let kind = null;
      this.axios
        .post(`/platformService/sys/dict/getDicts`, { kind: kind })
        .then(
          res => {
            if (res.data.code === 0) {
              this.dictListOrigData = res.data.data.data;
              this.dictListData = JSON.parse(
                JSON.stringify(this.dictListOrigData)
              );
            } else {
    ...
    getTheDict: function() {
      this.axios
        .post(`/platformService/sys/dict/getTheDict`, {
          id: this.selectedDictId
        })
        .then(
          res => {
            if (res.data.code === 0) {
    ...
    handleCurrentChange(val) {
      ...
      // 加载右侧数据
      this.axios
        .post(`/platformService/sys/dict/getDictValueVos`, {
    ...
    executeDel4Right: function() {
      this.axios
        .post(`/platformService/sys/dict/deleteDictValue`, {
          id: this.currentRow4Right.id,
          dictId: this.currentRow4Right.dictId
        })
        .then(
          res => {
            this.editLoading = false;
    ...
    getTheDictValue: function() {
      this.axios
        .post(`/platformService/sys/dict/getTheDictValue`, {
          id: this.currentRow4Right.id,
          dictId: this.currentRow4Right.dictId
        })
        .then(
    ...
    editDictValueSubmit: function() {
      this.$refs.editDictValueForm.validate(valid => {
        if (valid) {
          this.editLoading = true;
          this.axios
            .post(`/platformService/sys/dict/updateDictValue`, this.dictValue)
            .then(
              res => {
                this.editLoading = false;
                if (res.data.code === 0) {
