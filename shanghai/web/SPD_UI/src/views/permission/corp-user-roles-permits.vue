...
export default {
    ...
    methods: {
        ...
        loadMenu(){
            var treeData = [];
            var menuData = this.getUserMenuTree;
            menuData.forEach(topMenu=>{
                let topCanAdd = true;
                let secList=[];
                topMenu.children.forEach(secMenu=>{
                    let pageList = secMenu.children.filter(o=>{return o.requireAuth == "1"});
                    if(pageList.length > 0){
                        let secInfo = Object.assign({},secMenu);
                        secInfo.children = pageList;
                        secList.push(secInfo);
                    }
                });                    
                if(secList.length>0){
                    let topInfo = Object.assign({},topMenu);
                    topInfo.children = secList;
                    treeData.push(topInfo);
                }
            });
            this.menusTree.data = treeData;
        },             
