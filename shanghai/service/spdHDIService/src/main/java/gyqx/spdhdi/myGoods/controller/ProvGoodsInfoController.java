...
public class ProvGoodsInfoController extends BaseController {
	...
	public AjaxResult getGoodsInfos(HttpServletRequest request, HttpServletResponse response, @RequestBody QueryInfo<ProvGoodsInfo> queryInfo)
			throws Exception {
		ProvGoodsInfo provGoodInfo = queryInfo.getQueryObject();
		//如果有查询条件 则根据 条件来查询对应的供应商品
		if(provGoodInfo.getProvId()== null){
			// 只能查询本供应商的商品  
	...
	@RequestMapping(value = "getGoodsOutHos")
	@ResponseBody
	public AjaxResult getGoodsOutHos(HttpServletRequest request, HttpServletResponse response, @RequestBody QueryInfo<ProvGoodsHosVo> queryInfo)
			throws Exception {
		ProvGoodsHosVo provGoodInfo = queryInfo.getQueryObject();
		//如果有查询条件 则根据 条件来查询对应的供应商品
		if(provGoodInfo.getProvId()== null){
			// 只能查询本供应商的商品  
			queryInfo.getQueryObject().setProvId(userOnline.getCurrent().getCorpId());
		}
		QueryResult<ProvGoodsInfoVo> queryResult=   provGoodsInfoService.listProvGoodsOuthos(queryInfo);
		AjaxResult result = new AjaxResult();
		result.setData(queryResult);
		return result;
	}
	...
	//	String classPath = Thread.currentThread().getContextClassLoader().getParent().getResource("/").toString();
	   ...
	   // log.info("classPath:"+classPath);
	    ...
	    log.info("主目录:"+request.getRealPath("/"));
	         
	   // File file=new File(webApp+"/docs");
	...
	public AjaxResult importExcel(HttpServletRequest request, HttpServletResponse response,@RequestBody Map map )
		...
		if (!StringUtils.isEmpty(map.get("id"))) {
			...
			String fileName = "import2003_ProvGoods"+map.get("id") + "." + map.get("ext");
	...
	private void changeVo( List<ProvGoodsInfoVo> data,String[] columns ) throws Exception{
		...
		for (int i=0;i<data.size();i++){
				...
				log.info("开始遍历数据："+i+"-->"+p.getId());
				for(int j=0;j<columns.length;j++){
					log.info("    开始遍历columns："+columns[j]);
					   for(int k=0;k<fields.length;k++){
							...
							String theFieldtTitle=	EntityUtils.getFieldCName(p.getClass(), theField);
							   ...
							   if(theFieldtTitle.equals(columns[j])){
								   log.info("                开始处理："+i+"-->"+p.getId()+"-->"+theFieldtTitle);
								   break;
							   }		
					   }
					   log.info("    结束遍历columns："+columns[j]);
				}
		}
	}
	...
	//	hosGoodsInfoService.addHosGoodS(goods);
		log.info(String.valueOf(basGoods.size()));
    ...
    private Map convert2ProvGoodsInfo(List<Object> kk,ProvGoodsInfo pp){
		  ...
		  /*  if(  !StringUtils.isEmpty(kk.get(14)))
		    ...
		    if(  !StringUtils.isEmpty(kk.get(16)))
		    	pp.setBrand(kk.get(16).toString());
