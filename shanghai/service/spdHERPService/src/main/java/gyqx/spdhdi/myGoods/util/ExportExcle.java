...
public class ExportExcle<T> {
	...
	public void exportExcel(String title, String[] headers,
		...
		while (it.hasNext()) {
			...
			System.out.println("开始遍历数据："+index+"-->");
			for(int j=0;j<headers.length;j++){
				System.out.println("    开始遍历columns："+headers[j]);
				   for(int k=0;k<fields.length;k++){
						...
						String theFieldtTitle=	EntityUtils.getFieldCName(t.getClass(), theField);
						   ...
						   if(theFieldtTitle.equals(headers[j])){
							   System.out.println("                开始处理："+index+"-->"+theFieldtTitle);
				   ...
				   System.out.println("    结束遍历columns："+headers[j]);
			}
			System.out.println("结束遍历数据："+index+"-->");
